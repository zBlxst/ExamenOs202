...
if __name__ == '__main__':    
    ...
    shape = values_gray.shape
    values_gray = values_gray[rank*values_gray.shape[0]//nbp:(rank+1)*values_gray.shape[0]//nbp,:]
    ...
    val_ycbcr = val_ycbcr[rank*val_ycbcr.shape[0]//nbp:(rank+1)*val_ycbcr.shape[0]//nbp,:]
    ...
    new_Cb = np.asarray(new_Cb, dtype=int)
    new_Cr = np.asarray(new_Cr, dtype=int)
    intensity = np.asarray(intensity[2:-2,2:-2], dtype=int)
    
        
    new_Cb = globCom.reduce(new_Cb, lambda x, y: np.concatenate((x, y)), root=0)
    new_Cr = globCom.reduce(new_Cr, lambda x, y: np.concatenate((x, y)), root=0)
    intensity = globCom.reduce(intensity, lambda x, y: np.concatenate((x, y)), root=0)
    ...
    # Puis on sauve l'image dans un fichier :
        ...
        new_image_array[:,:,1] = np.reshape(new_Cb, shape[:2]).astype('uint8')
        new_image_array[:,:,2] = np.reshape(new_Cr, shape[:2]).astype('uint8')
