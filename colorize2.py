...
def parallelized_prod(A, B, continue_=True):
    if rank == 0:
        if not continue_:
            globCom.bcast(False, root=0)
        else:
            globCom.bcast(True, root=0)
            A_ = A[:]
            print(A.shape)
            A = [A[r*A.shape[0]//nbp:(r+1)*A.shape[0]//nbp, :] for r in range(nbp)]
            A_loc = globCom.scatter(A, root=0)
            B = globCom.bcast(B, root=1)
            C = A_loc.dot(B)
            print(rank, A_loc.shape)
            print(rank, B.shape)
            print(rank, C.shape)
            C = globCom.gather(C, root=0)
            print(C[0].shape)
        ...
        while globCom.bcast(0, root=0):
            A_loc = globCom.scatter(0, root=0)
            B = globCom.bcast(B, root=1)
            C = A_loc.dot(B)          
            print(rank, A_loc.shape)
            print(rank, B.shape)
    ...
    # On suppose que la matrice A n'est pas encore divisée par ligne
    # A = A[rank*A.shape[0]//nbp:(rank+1)*A.shape[0]//nbp, :]
    # C = A.dot(B)
    # C = globCom.allreduce(C, lambda x, y: np.concatenate((x, y)))
    # return C
...
if __name__ == '__main__':
    ...
    parallelized_prod(A, A, True)
    if rank == 0:
        parallelized_prod(0, 0, False)
    exit()
